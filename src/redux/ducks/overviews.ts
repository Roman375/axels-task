...
export const overview = (state: StateType = [], action: any): Array<object> => {
  switch (action.type) {
    case SET_OVERVIEW:
...
function* workerloadOverviews(employeeName: object): any  {
  const payload: Array<object>  = yield call(fetchOverview , employeeName)
  yield put(setOverview(payload))
}

async function fetchOverview(args: any) {
  const employeeName: string = args.employeeName[0]
